...
1. Viết pattern regex
   ...
   - Mã PIN (6 chữ số, không hiển thị): `type="password" pattern="[0-9]{6}"
2. HTML5 chưa đủ an toàn cho ngân hàng vì HTML5 chỉ là lớp bảo vệ bề mặt (Frontend).
Người dùng có thể dễ dàng sử dụng các công cụ để xóa bỏ các thuộc tính required, pattern.
Nó chỉ mang tính chất hỗ trợ trải nghiệm người dùng chứ không phải lớp bảo mật thực thụ.
3. 3 loại validation HTML5 không thể làm được
   - So sánh khớp dữ liệu (Kiểm tra xác nhận mật khẩu có trùng khớp với mật khẩu hay không)
   - Kiểm tra tính duy nhất (kiểm tra emailhay username đã tồn tại trong cơ sở dữ liệu hay chưa)
   - Validation phụ thuộc (nếu chọn phunogw thức xác nhận là mã OTP qua email thì mới bắt buộc nhập email, còn chọn xác nhận qua số điện thoại thì không cần)
4. 2 rủi ro bảo mật nếu chỉ validate trên frontend
   - Dữ liệu độc hại tấn công database
   - Phá vỡ quy tắc logic kinh doanh: người dùng có thể gửi số tiền âm trong giao dịchchuyeenr khoản hoặc nhập số CCCD giả mạo.
